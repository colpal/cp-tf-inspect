...
func TestCLIListModuleSource_JSON_RecursiveWrapper(t *testing.T) {
	binary := "../cp-tf-inspect"
	if _, err := exec.LookPath(binary); err != nil {
		t.Skip("cp-tf-inspect binary not found; build it before running integration tests")
	}

	workspace := filepath.Join("fixtures", "wrapper")
	cmd := exec.Command(binary, "list-module-source", "--dir", workspace, "--recursive")
	outBytes, err := cmd.CombinedOutput()
	out := string(outBytes)
	if err != nil {
		t.Fatalf("error running CLI: %s\noutput: %s", err, out)
	...
	for _, w := range want {
		if !strings.Contains(out, w) {
			t.Errorf("Output missing expected module source (recursive wrapper): %s\nfull output:\n%s", w, out)
		}
	}
}
